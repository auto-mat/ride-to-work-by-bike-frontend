...
<template>
  <div>
    ...
    <div class="row q-col-gutter-lg q-row-gutter-md" data-cy="list-card-offer">
      <div v-for="card in renderedCards" :key="card.title" class="col-12 col-sm-6 col-lg-4"
    ...
    <div v-if="hasMoreCards" class="text-center">
      <q-btn rounded color="black" unelevated outline :label="$t('index.cardListOffer.button', { count: cards?.length })"
        class="q-mt-md" data-cy="list-card-offer-button" />
    </div>
  </div>
</template>
