      ...
      testCase.displayData.forEach((box, boxIndex) => {
        cy.dataCy('table-boxes-row')
          ...
          .within(() => {
            ...
            // last modified date
            if (box.lastModified) {
              cy.dataCy('table-boxes-last-modified').should(($td) => {
                const expectedText = i18n.global.d(
                  new Date(box.lastModified),
                  'numeric',
                );
